...
def create_master(gmpp_wb, latest_data, last_gmpp):
    ...
    gmpp_project_names = []
    for i, name in enumerate(latest_data):
        if latest_data[name]['GMPP - IPA DCA'] != None:
            gmpp_project_names.append(name)

    for i, name in enumerate(gmpp_project_names):
        print(name)
        ws.cell(row=1, column=11+i).value = name  # place project names in file

        # for loop for placing data into the worksheet
        for row_num in range(2, ws.max_row+1):
            key = ws.cell(row=row_num, column=9).value
            # this loop places all latest raw data into the worksheet
            if key in latest_data[name].keys():
                ws.cell(row=row_num, column=11+i).value = latest_data[name][key]
                ...
                for cost_type in type_list_2:
                    if cost_type in key:
                        if latest_data[name][key] is None:
                            ws.cell(row=row_num, column=11 + i).value = 0
...
latest_dm = load_workbook("C:\\Users\\Standalone\\Will\\masters folder\\gmpp_reporting_docs\\datamaps\\"
                          "gmpp_datamap_FINAL.xlsx")    # 1) place file path to gmpp data map here

dft_master = project_data_from_master("C:\\Users\\Standalone\\Will\\masters folder\\gmpp_reporting_docs\\"
                                      "gmpp_master_Q3_EWR.xlsx")
# 2) place file path to latest quarter master above

last_gmpp_master = project_data_from_master('C:\\Users\\Standalone\\Will\\masters folder\\gmpp_reporting_docs\\Q2_1819'
...
run.save("C:\\Users\\Standalone\\Will\\masters folder\\gmpp_reporting_docs\\gmpp_Q3_2018_19_EWR.xlsx")
